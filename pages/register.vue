    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 sm:p-10">
      ...
      <form @submit.prevent="handleRegister" class="space-y-5">
        ...
        <div>
          ...
          <div class="relative">
            <input v-model="form.password" :type="showPassword ? 'text' : 'password'" class="input pr-12"
              placeholder="Minimal 8 karakter" required minlength="8" :disabled="loading" />
          ...
          <div v-if="form.password" class="mt-2">
            <div class="flex gap-1">
              <div v-for="i in 4" :key="i"
                :class="['h-1 flex-1 rounded-full transition-colors', i <= passwordStrength.score ? passwordStrength.color : 'bg-gray-200']" />
            </div>
        ...
        <div>
          ...
          <input v-model="form.confirmPassword" :type="showPassword ? 'text' : 'password'" class="input"
            :class="{ 'border-red-400': form.confirmPassword && !passwordsMatch }" placeholder="Ulangi password"
